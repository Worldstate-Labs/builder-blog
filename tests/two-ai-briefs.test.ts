...
test("Sources shows only the user's AI Brief while FollowBrief remains available to readers", () => {
  ...
  assert.doesNotMatch(buildersPage, /followBriefDigestPipeline|FollowBriefDigestPipelineCard/);
...
test("Sources AI Brief page uses one direct user card without collection panels", () => {
  const buildersPage = source("src/app/(workspace)/builders/page.tsx");
  ...
  assert.doesNotMatch(buildersPage, /<FollowBriefDigestPipelineCard/);
