...
test("favorites saves posts into the Favorites tab", () => {
  ...
  assert.match(feedFavorites, /Post is not in your sources or imported AI Digest collections/);
  assert.doesNotMatch(feedFavorites, /Post is not in your sources or imported AI Digest issues/);
...
test("following recommendation feed uses subscribed builders only", () => {
  ...
  assert.match(followingSection, /Add sources in the Sources tab\. They feed AI Digest issues and Following posts\./);
  assert.doesNotMatch(followingSection, /Add sources in Sources\. They feed AI Digest|They feed AI Digest and Following\./);
  ...
  assert.doesNotMatch(followingSection, /Use Fetch sources to summarize followed posts|Following recommendations can appear|No unread recommendations yet|No posts have been fetched for your followed sources yet|fetching recommendations/);
