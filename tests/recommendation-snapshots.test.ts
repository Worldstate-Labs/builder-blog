...
test("favorites saves posts into the Favorites tab", () => {
  ...
  assert.match(schema, /markedReadAt DateTime\?/);
  assert.match(schema, /@@index\(\[userId, markedReadAt\]\)/);
  ...
  assert.match(favoriteReadRoute, /export async function PATCH/);
  assert.match(favoriteReadRoute, /markFavoritePostRead/);
  assert.match(favoriteReadRoute, /markedReadAt:\s*markedReadAt\?\.toISOString\(\) \?\? null/);
  assert.doesNotMatch(favoriteReadRoute, /FeedRead|feedRead/);
  assert.match(favoriteSection, /feedFavorite\.findMany/);
  assert.match(favoriteSection, /orderBy:\s*\[/);
  assert.match(favoriteSection, /markedReadAt:\s*\{ sort: "asc", nulls: "first" \}/);
  assert.match(favoriteSection, /favoritedAt: "desc"/);
  assert.match(favoriteSection, /take:\s*favoritePostLimit/);
  assert.match(favoriteSection, /markedReadAt: favorite\.markedReadAt\?\.toISOString\(\) \?\? null/);
  assert.match(favoriteSection, /sortFavoriteItems\(items\)/);
  assert.doesNotMatch(favoriteSection, /feedRead\.findMany|readAtByKey|favoriteIdentityKey/);
  ...
  assert.match(favoriteList, /method: "PATCH"/);
  assert.match(favoriteList, /markedReadAt: optimisticMarkedReadAt/);
  assert.match(favoriteList, /runFavoriteListTransition/);
  assert.match(favoriteList, /FAVORITE_READ_REORDER_DELAY_MS/);
  ...
  assert.doesNotMatch(feed, /disabled=\{isRead\}|mode\?: "favorites"|\/api\/favorites\/read|markedReadAt/);
  assert.match(postCard, /favoriteMarkedRead\?: boolean/);
  assert.match(postCard, /data-favorite-read=\{favoriteMarkedRead \? "true" : undefined\}/);
  assert.match(globals, /\.fetched-post-card\[data-favorite-read="true"\]\s*{/);
  assert.match(globals, /\.favorite-mark-read\s*{/);
  assert.match(globals, /inset 4px 0 0/);
  assert.match(globals, /linear-gradient\(\s*90deg/);
...
test("post detail keeps source actions with the source row without a top source type badge", () => {
  ...
  assert.doesNotMatch(html, /Saved by Fetch sources\.|post-detail-section-desc/);
...
test("recommendation snapshots request six posts at a time", () => {
  ...
  assert.match(apiRoute, /beforeItemId/);
  assert.match(apiRoute, /afterPublishedAt/);
  ...
  assert.match(feed, /newestPublishedCursor/);
  assert.match(feed, /direction === "append" \? "before" : "after"/);
  assert.match(feed, /params\.set\(`\$\{prefix\}PublishedAt`, cursor\.publishedAt\)/);
  assert.match(feed, /params\.set\(`\$\{prefix\}ItemId`, cursor\.itemId\)/);
  assert.match(feed, /followbrief\.following\.last-position\.v1/);
  assert.match(feed, /Jump to last position/);
  assert.match(feed, /RecommendationPositionMarker/);
  assert.match(feed, /IntersectionObserver/);
  assert.match(feed, /showResumeJump[\s\S]*suppressPositionWritesRef/);
  assert.match(feed, /scrollToFollowingItem/);
