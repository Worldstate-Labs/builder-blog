...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(globals, /\.access-key-card\s*{[\s\S]*min-height:\s*4.7rem/);
  ...
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-card\s*{[\s\S]*min-height:\s*4\.7rem/);
  ...
  assert.match(globals, /\.access-key-device-title\s*{[\s\S]*font-size:\s*1\.0625rem/);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(settingsPage, /Runs before AI Digest: discover source posts, filter candidates, and\s*write per-post summaries\./);
  ...
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-card--skeleton\s*{[\s\S]*min-height:\s*4\.7rem/);
