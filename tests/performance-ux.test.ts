...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(adminDigestConfig, /Creates the headline summary in the selected AI Digest language/);
  ...
  assert.match(adminSourceTypeManager, /Defines each post summary/);
  assert.match(adminSourceTypeManager, /Language comes from the run prompt/);
...
test("desktop shell uses centered top navigation and merged home feeds", () => {
  ...
  assert.match(dashboardPage, /Add sources in the Sources tab\. They feed AI Digest issues and Following posts\./);
  assert.doesNotMatch(dashboardPage, /Add sources in Sources\. They feed AI Digest|They feed AI Digest and Following\./);
  ...
  assert.match(builderDetailPage, /Use Fetch sources to summarize this source's posts\./);
  ...
  assert.doesNotMatch(builderDetailPage, /actions=\{[\s\S]*href="\/builders\?tab=fetch"[\s\S]*(Open Sources|Run Fetch sources)/);
...
test("search page uses a client form with pending feedback", () => {
  ...
  assert.match(searchPage, /All result types/);
  assert.match(searchPage, /Any time/);
  assert.match(searchPage, /label:\s*"Clear filters"/);
  assert.doesNotMatch(searchPage, /Clear all(?: filters)?/);
  assert.doesNotMatch(searchPage, /Search all result types|Search any time|Search all time/);
...
test("builders page exposes per-builder fetched posts ordered by time", () => {
  ...
  assert.match(buildersPage, /emptyBody="Add sources, then run Fetch sources\. They feed AI Digest issues and Following posts\."/);
  assert.doesNotMatch(buildersPage, /emptyBody="Add a source, then run Fetch sources\."/);
  assert.doesNotMatch(buildersPage, /AI Digest and Following use this library|They feed AI Digest and Following/);
  ...
  assert.match(builderFeedItems, /Use Fetch sources to summarize this source's posts\./);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(settingsPage, /AI Digest rules/);
  assert.match(settingsPage, /Used when building AI Digest issues\./);
  ...
  assert.match(tokenPanel, /Name this access key so you can recognize it later/);
  assert.doesNotMatch(tokenPanel, /Give this access key a name so you can recognize it later/);
