...
test("public entry pages use the centered product layout", () => {
  ...
  assert.match(landingPage, /reading an AI Digest issue/);
  ...
  assert.match(landingPage, /Unread posts from followed sources stay separate from AI Digest issues\./);
  assert.doesNotMatch(landingPage, /Unread posts from followed sources stay separate from the AI Digest\./);
...
test("primary tabs keep local loading fallbacks alongside route loaders", () => {
  ...
  assert.match(libraryHubPage, /Import shared AI Digest collections into AI Digest\./);
  assert.doesNotMatch(libraryHubPage, /AI Digest tab|Import shared collections into AI Digest\./);
  ...
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /Source libraries imported into Sources\./);
  ...
  assert.doesNotMatch(source("src/components/LibraryHubImportForm.tsx"), /Import source libraries to see them in the Sources tab\.|Import source libraries to see them in Sources\./);
  ...
  assert.match(source("src/components/DigestPipelineImportForm.tsx"), /\?\s*"Importing collection"\s*:\s*"Import collection"/);
...
test("builders page exposes per-builder fetched posts ordered by time", () => {
  ...
  assert.doesNotMatch(sourceValueDetect, /can't be imported|doesn't expose| URL — switch|podcasts\.apple\.com\/\.\.\.|id\.\.\.|show's RSS feed URL/);
...
test("library hub exposes share and multi-import flows", () => {
  ...
  assert.match(hubImportForm, /Import it again from Hub\./);
  ...
  assert.match(libraryImportRemoveButton, /Import it again from Hub\./);
  ...
  assert.match(digestPipelineForm, /Import shared AI Digest collections into AI Digest\./);
  assert.match(digestPipelineForm, /Available in AI Digest\./);
  assert.doesNotMatch(digestPipelineForm, /AI Digest tab|Import shared collections into AI Digest\.|Already in AI Digest\.|Collections already in AI Digest\./);
  ...
  assert.match(digestPipelineForm, /Import AI Digest collections to see them here\./);
  assert.doesNotMatch(digestPipelineForm, /Import one to see it in AI Digest\.|Import collections to see them in AI Digest\.|Import archives to see them in AI Digest\./);
  ...
  assert.match(digestPipelineForm, /\?\s*"Importing collection"\s*:\s*"Import collection"/);
  assert.doesNotMatch(digestPipelineForm, /aria-label=\{`Import \$\{pipeline\.title\}`\}|: "Import AI Digest archive"|Importing AI Digest archive|Import archive/);
