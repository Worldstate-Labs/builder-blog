...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(globals, /\.access-keys-list--skeleton \.settings-skeleton-row\s*{[\s\S]*height:\s*5rem/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-keys-list--skeleton \.settings-skeleton-row\s*{[\s\S]*height:\s*5\.5rem/);
