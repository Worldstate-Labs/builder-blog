...
test("desktop shell uses centered top navigation and merged home feeds", () => {
  ...
  assert.match(dashboardPage, /<PageHeader[\s\S]*title="Today"[\s\S]*Read AI Digest issues and Following posts, then save or open originals\./);
...
test("dashboard digest tab owns the AI Digest issue selector", () => {
  ...
  assert.match(digestHeadlineSummary, /parseDigest/);
  assert.match(digestHeadlineSummary, /headlineSourceOrderFromDigestContent/);
