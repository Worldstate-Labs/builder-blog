...
test("desktop shell uses centered top navigation and merged home feeds", () => {
  ...
  assert.match(builderDetailPage, /headerHostLabel \? \([\s\S]*className="builder-detail-host source-host-meta mono"[\s\S]*className="source-count-dot source-meta-dot"/);
...
test("builders page exposes per-builder fetched posts ordered by time", () => {
  ...
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.builder-library-card\s*{[\s\S]*grid-template-columns:\s*2\.25rem minmax\(0,\s*1fr\)/);
...
test("list actions use compact controls instead of full-width mobile buttons", () => {
  ...
  assert.match(css, /\.builder-library-card\s*{[\s\S]*grid-template-columns:\s*2rem minmax\(0,\s*1fr\)/);
  ...
  assert.match(css, /@media \(max-width:\s*767px\)[\s\S]*\.builder-library-card\s*{[\s\S]*grid-template-columns:\s*2\.25rem minmax\(0,\s*1fr\)/);
