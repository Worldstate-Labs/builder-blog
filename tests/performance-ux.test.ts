...
test("public entry pages use the centered product layout", () => {
  ...
  assert.match(landingPage, /Read them as one AI Digest issue\./);
  assert.doesNotMatch(landingPage, /Read them as one AI Digest\./);
...
test("dashboard defers heavy recommendation timeline work to a client island", () => {
  ...
  assert.doesNotMatch(favoriteList, /post pages|to build this queue|Use the star on any post in AI Digest, Following, Search, or a post detail page to build a focused reading queue here\./);
...
test("primary tabs keep local loading fallbacks alongside route loaders", () => {
  ...
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /Shared source libraries appear here\./);
...
test("library hub exposes share and multi-import flows", () => {
  ...
  assert.match(hubImportForm, /its sources are\s+removed from Sources and stop feeding AI Digest and Following\./);
  ...
  assert.match(hubImportForm, /You can import it from Hub again\./);
  ...
  assert.match(libraryImportRemoveButton, /its sources are removed\s+from Sources and stop feeding AI Digest and Following\./);
  ...
  assert.match(libraryImportRemoveButton, /You can import it from Hub again\./);
  ...
  assert.match(digestPipelineForm, /Imported collections appear here\./);
  ...
  assert.match(digestPipelineForm, /Shared collections appear here\./);
  assert.doesNotMatch(digestPipelineForm, /AI Digest collections shared to Hub appear here\.|AI Digest collections appear here after users share them to Hub\.|Shared collections appear here after users share them to Hub\.|Shared AI Digest archives appear here when users share them to Hub\.|Archives appear here after users share them to Hub\.|Shared AI Digest collections appear here after they are shared to Hub\.|Shared AI Digest collections appear here when people share them to Hub\./);
  assert.match(digestPipelineForm, /No AI Digest issues yet/);
  assert.doesNotMatch(digestPipelineForm, /No AI Digest archives yet/);
  ...
  assert.match(digestPipelineForm, /this collection\s+no longer appears in AI Digest\./);
  ...
  assert.match(digestPipelineForm, /You can import it from Hub again\./);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(tokenPanel, /Give this key a recognizable name/);
  assert.doesNotMatch(tokenPanel, /Name this access key to recognize it later|Give this access key a name so you can recognize it later|Name this access key so you can recognize it later/);
  ...
  assert.match(tokenPanel, /Create one to connect a device or Local Agent\./);
  ...
  assert.match(tokenPanel, /Unused key\. No device or Local Agent loses access\./);
  assert.match(tokenPanel, /Create a new access key for future updates\./);
  assert.match(tokenPanel, /No device or Local Agent\s+loses\s+access/);
  ...
  assert.doesNotMatch(tokenPanel, /Create one before connecting a device or Local Agent|This access key has been used by|This key has never been used|It needs a new access key to update\s+FollowBrief|That device or Local Agent needs a new access key|After revoking it, that device or Local Agent will need|will need a new access key|Revoking it now is safe|that Local Agent will lose access|will lose access|No machine will lose access/);
