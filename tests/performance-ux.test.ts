...
test("desktop shell uses centered top navigation and merged home feeds", () => {
  ...
  assert.match(globals, /\.fb-nav-list-rail \.fb-nav\s*{[\s\S]*flex-direction:\s*column/);
...
test("digest posts use source detail headings and unified original links", () => {
  ...
  assert.match(globals, /\.digest-rich \.fetched-post-card\.feed-card\s*{[\s\S]*margin-inline:\s*1\.8rem/);
  assert.match(globals, /\.digest-rich \.fetched-post-card\.feed-card\s*{[\s\S]*width:\s*calc\(100% - 3\.6rem\)/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.digest-rich \.fetched-post-card\.feed-card\s*{[\s\S]*margin-inline:\s*0\.75rem/);
