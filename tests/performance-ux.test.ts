...
test("workspace auto-refresh covers server-side data changes without manual reloads", () => {
  ...
  assert.match(digestLogPanel, /Prepared, no AI Brief saved/);
  ...
  assert.match(digestLogPanel, /Found, AI Brief not saved yet/);
  ...
  assert.doesNotMatch(digestLogPanel, /aria-label="Digest update views"|>Digest updates<|Show digest update details|Hide digest update details|aria-label="Digest schedule status graph"|scheduled brief run|already digested items|past items|new items|not save a digest|Untitled digest|Prepared, no digest saved|Previous digest|Found, digest not saved yet|Used in the digest/);
