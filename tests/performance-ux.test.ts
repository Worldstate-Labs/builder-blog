...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(globals, /\.access-key-card\s*{[\s\S]*border-radius:\s*12px/);
...
test("dashboard digest tab owns the saved AI Digest selector", () => {
  ...
  assert.match(buildersPage, /ariaLabel="Source Library and AI Digest"/);
  assert.match(buildersPage, /label:\s*"Source Library"[\s\S]*href:\s*"\/builders"/);
  assert.doesNotMatch(buildersPage, /label:\s*"Sources"[\s\S]*href:\s*"\/builders"/);
...
test("primary tabs keep local loading fallbacks alongside route loaders", () => {
  ...
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /shortLabel:\s*"All sources"/);
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /shortLabel:\s*"My sources"/);
  ...
  assert.doesNotMatch(source("src/components/LibraryHubImportForm.tsx"), /Library filter|All libraries|My libraries|No libraries match this filter yet|\bYour library\b|label\.replace/);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(globals, /\.access-keys-list--skeleton \.settings-skeleton-row\s*{[\s\S]*height:\s*4\.5rem/);
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*border-radius:\s*999px/);
