...
test("dashboard defers heavy recommendation timeline work to a client island", () => {
  ...
  assert.match(feedState, /className="feed-state-panel"/);
...
test("primary tabs use local loading fallbacks instead of full-route loaders", () => {
  ...
  assert.match(source("src/app/globals.css"), /\.fb-hub-card-actions\s*{[\s\S]*position:\s*relative/);
  assert.match(source("src/app/globals.css"), /\.digest-remove-popover\s*{[\s\S]*position:\s*absolute/);
  assert.match(source("src/app/globals.css"), /\.digest-remove-popover\s*{[\s\S]*top:\s*calc\(100% \+ 0\.45rem\)/);
  assert.match(source("src/app/globals.css"), /\.digest-remove-popover\s*{[\s\S]*right:\s*0/);
...
test("library hub exposes share and multi-import flows", () => {
  ...
  assert.doesNotMatch(digestPipelineForm, /className="fb-dialog"/);
  assert.match(digestPipelineForm, /className="digest-remove-popover"/);
  assert.match(digestPipelineForm, /role="dialog"/);
  assert.match(digestPipelineForm, /Remove imported digest\?/);
  assert.match(digestPipelineForm, /you will no\s+longer see this digest on the Home page/);
