...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(globals, /\.access-key-card\s*{[\s\S]*min-height:\s*7\.5rem/);
...
test("primary tabs keep local loading fallbacks alongside route loaders", () => {
  ...
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /Try another source library filter\./);
...
test("library hub exposes share and multi-import flows", () => {
  ...
  assert.match(digestPipelineForm, /No imported AI Digests/);
  assert.match(digestPipelineForm, /Import an AI Digest from Hub to see it on Home\./);
  ...
  assert.match(digestPipelineForm, /Remove AI Digest import\?/);
  ...
  assert.match(digestPipelineForm, /aria-label=\{`Remove \$\{pipeline\.title\} AI Digest import`\}/);
  assert.match(digestPipelineForm, />\s*Remove import\s*<\/button>/);
  assert.match(digestPipelineForm, /Could not import AI Digest/);
  assert.match(digestPipelineForm, /Could not remove AI Digest import/);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(globals, /\.access-keys-list--skeleton \.settings-skeleton-row\s*{[\s\S]*height:\s*7\.5rem/);
  ...
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-card\s*{[\s\S]*grid-template-columns:\s*2\.125rem minmax\(0,\s*1fr\) auto/);
