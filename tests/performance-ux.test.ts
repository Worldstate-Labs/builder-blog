...
test("every app route has an explicit centered layout role", () => {
  ...
  const publicRoutes = [
    ["src/app/page.tsx", /<PublicHeader current="home" \/>[\s\S]*<main className="lp">/],
  ...
  const workspaceRoutes = [
    ...
    ["src/app/(workspace)/library-hub/page.tsx", /className="page-pad"[\s\S]*<PageHeader[\s\S]*title=\{<I18nText id="workspace\.hub" \/>\}[\s\S]*className="workspace-content-stack"[\s\S]*<LibraryHubImportSection/],
  ] as const;
  ...
  assert.match(
    source("src/app/(workspace)/library-hub/loading.tsx"),
    /className="workspace-content-stack"/,
  );
  assert.doesNotMatch(
    source("src/app/(workspace)/library-hub/loading.tsx"),
    /workspace-content-stack--tabs-first|WorkspaceTabShell|ai-digests/,
  );
  ...
  assert.doesNotMatch(
    source("src/app/(workspace)/settings/page.tsx"),
    /Manage access keys and rules for Fetch sources and AI Brief\.|Access keys, source fetching, and AI Brief rules\.|Manage access keys, source fetching, and AI Brief rules\.|Manage Local Agent access and the rules used by Fetch sources and AI Brief\.|Manage access keys and the rules used by Fetch sources and AI Brief\.|Manage access keys and rules for Fetch sources and AI Briefs\./,
  );
  ...
  assert.doesNotMatch(settingsLoading, /Manage access keys and rules for Fetch sources and AI Brief\.|Access keys, source fetching, and AI Brief rules\.|Manage access keys, source fetching, and AI Brief rules\.|Manage access keys and the rules used by Fetch sources and AI Brief\.|Manage access keys and rules for Fetch sources and AI Briefs\./);
  ...
  assert.match(settingsRulesSkeleton, /AI Brief rules/);
  ...
  assert.doesNotMatch(dashboardLoading, /aria-label="Loading AI Brief selection"/);
  ...
  assert.match(buildersLoading, /aria-label="Sources and AI Brief tabs"/);
  ...
  assert.match(hubLoading, /className="workspace-content-stack"/);
  assert.doesNotMatch(hubLoading, /Hub tabs|tablist|role="tab"|tabs\.aiDigestCollections/);
  assert.doesNotMatch(hubLoading, /hub-loading-tab|tabs\.aiDigestCollections|Loading Hub content/);
  assert.match(hubLoading, /<span className="sr-only">Loading source libraries<\/span>/);
...
test("workspace not-found uses the FollowBrief shell instead of the default Next 404", () => {
  ...
  assert.match(notFoundPage, /This page is unavailable or outside your sources or AI Briefs\./);
  assert.doesNotMatch(notFoundPage, /This page may be unavailable|outside your Sources and AI Brief collections|imported AI Brief collections|This page may have moved, been removed, or be outside your Sources or imported AI Brief collections\./);
  ...
  assert.match(notFoundPage, /Search sources, posts, and AI Briefs\./);
  assert.doesNotMatch(notFoundPage, /search across sources, posts, saved posts, and AI Briefs\./);
  assert.doesNotMatch(notFoundPage, /search across sources, posts, saved posts, and AI Briefs\./);
  assert.doesNotMatch(notFoundPage, /sources, posts, Favorites, and AI Briefs/);
...
test("public entry pages use the centered product layout", () => {
  ...
  assert.match(landingPage, /<main className="lp">/);
  assert.match(landingPage, /className="lp-hero-kicker"/);
  assert.match(landingPage, /<I18nText id="home\.kicker" \/>/);
  assert.match(landingPage, /className="lp-hero-title"/);
  assert.match(landingPage, /<I18nText id="home\.heroTitle1" \/>/);
  assert.match(landingPage, /<I18nText id="home\.heroTitle2" \/>/);
  assert.match(landingPage, /<I18nText id="home\.heroCopy" \/>/);
  assert.match(landingPage, /className="lp-hero-copy"/);
  ...
  assert.doesNotMatch(landingPage, /home\.loopNote|className="lp-section-note"/);
  assert.match(landingPage, /className="lp-steps"/);
  assert.match(landingPage, /<Rss aria-hidden="true" \/>/);
  assert.match(landingPage, /<Terminal aria-hidden="true" \/>/);
  assert.match(landingPage, /<Search aria-hidden="true" \/>/);
  assert.match(landingPage, /<I18nText id="home\.step1Title" \/>/);
  assert.match(landingPage, /<I18nText id="home\.step1Copy" \/>/);
  assert.match(landingPage, /<I18nText id="home\.step1Tag" \/>/);
  assert.match(landingPage, /<I18nText id="home\.step2Title" \/>/);
  assert.match(landingPage, /<I18nText id="home\.step2Copy" \/>/);
  assert.match(landingPage, /<I18nText id="home\.step2Tag" \/>/);
  assert.match(landingPage, /<I18nText id="home\.step3Title" \/>/);
  assert.match(landingPage, /<I18nText id="home\.step3Copy" \/>/);
  assert.match(landingPage, /<I18nText id="home\.step3Tag" \/>/);
  assert.match(landingPage, /className="lp-cta"/);
  assert.match(landingPage, /className="lp-cta-mark">F</);
  assert.match(landingPage, /className="lp-cta-name">FollowBrief</);
  assert.match(landingPage, /<I18nText id="home\.ctaTagline" \/>/);
  assert.match(landingPage, /className="lp-cta-btn" href="\/login"/);
  assert.doesNotMatch(landingPage, /className="lp-cta-url"|className="lp-footer"|© 2026 FollowBrief|href="#how-it-works"/);
  assert.match(landingPage, /import \{ Rss, Search, Terminal \} from "lucide-react"/);
  assert.doesNotMatch(landingPage, /fb-landing-grid|fb-public-hero|fb-public-flow|fb-product-demo|fb-public-workflow|fb-public-workspace/);
  assert.doesNotMatch(landingPage, /Newspaper|Radio/);
  ...
  assert.doesNotMatch(loginPage, /AI Brief workspace/);
  assert.doesNotMatch(loginPage, /Sign in to your/);
  assert.match(loginPage, /className="fb-login-title-break"/);
  assert.doesNotMatch(loginPage, /your<br \/>AI Brief workspace/);
  ...
  assert.doesNotMatch(loginPage, /Follow sources, build cited AI Briefs|Follow sources, read the cited AI Brief|Follow source libraries|Follow source libraries, read AI Briefs|read cited AI Briefs|search them\s*alongside sources and posts later|search sources,\s*posts, and AI Briefs|search sources,\s*posts, and issues|search them with\s*sources and posts/);
  assert.doesNotMatch(loginPage, /sources,\s*posts, and AI Briefs searchable/);
  assert.doesNotMatch(loginPage, /sources, posts, saved posts, and AI Briefs searchable/);
  assert.doesNotMatch(loginPage, /sources, posts, saved posts, and AI Briefs searchable/);
  assert.doesNotMatch(loginPage, /sources, posts, Favorites, and AI Briefs searchable/);
  assert.doesNotMatch(loginPage, /keep your\s+AI Briefs searchable/);
  ...
  assert.doesNotMatch(loginPage, /Use the same account for the app and Local Agent\.|Use one account for your AI Briefs and Local Agent\.|Use one account for FollowBrief and your Local Agent\.|Use the same FollowBrief account for the app and Local Agent\./);
  ...
  assert.match(rootLayout, /Follow sources, build AI Brief, and search your workspace\./);
  assert.doesNotMatch(rootLayout, /build cited AI Briefs/);
  assert.doesNotMatch(rootLayout, /Follow source libraries/);
  assert.doesNotMatch(rootLayout, /read the cited AI Brief|read cited AI Briefs|search them alongside sources and posts|search sources, posts, and AI Briefs|search sources, posts, and issues|search them with sources and posts/);
  assert.doesNotMatch(rootLayout, /AI Briefs from people and sources you follow\./);
  ...
  assert.match(globals, /\.lp-wrap\s*{[\s\S]*box-sizing:\s*border-box/);
  assert.match(globals, /@media \(max-width:\s*640px\)[\s\S]*\.lp-hero\s*{[\s\S]*padding:\s*56px 0 28px/);
  assert.match(globals, /@media \(max-width:\s*640px\)[\s\S]*\.lp-hero-actions\s*{[\s\S]*flex-direction:\s*column/);
  assert.match(globals, /@media \(max-width:\s*640px\)[\s\S]*\.lp-btn-lg\s*{[\s\S]*width:\s*100%/);
  assert.match(globals, /@media \(max-width:\s*640px\)[\s\S]*\.lp-cta-btn\s*{[\s\S]*width:\s*100%/);
  assert.doesNotMatch(globals, /\.lp-kinds|\.lp-kinds-label|\.lp-film-head|\.lp-film-note|\.lp-film-chrome-title|\.lp-section-note|\.lp-cta-url|\.lp-footer/);
...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(digestDetails, /View AI Brief/);
  ...
  assert.doesNotMatch(agentTokenPanel, /Update sources and AI Briefs|Copy Local Agent prompts|Revoke anytime/);
  ...
  assert.doesNotMatch(adminDigestConfig, /AI Brief prompts/);
  assert.doesNotMatch(adminDigestConfig, /Prompts used to generate AI Brief\./);
  ...
  assert.doesNotMatch(adminDigestConfig, /Creates the headline summary in the selected AI Brief language|Adds an optional source-level note above that source's posts|Creates post summaries in the selected AI Brief language without dropping key points/);
  assert.match(adminDigestConfig, /Could not save AI Brief rules\./);
  ...
  assert.match(digestConfigRoute, /Headline and per-source brief prompts can only be changed by an admin/);
  assert.match(digestConfigRoute, /Could not save AI Brief rules\./);
  ...
  assert.doesNotMatch(adminSourceTypeManager, /Failed posts are not used in Following or AI Brief|Posts that fail are not saved or used in Following or AI Brief|Checks extracted content before it reaches Following or AI Brief|Checks extracted content before it reaches Following posts or AI Briefs|Drop posts below this body length|Drop posts whose body has fewer characters than this|Drop posts with bodies shorter than this|Drop posts with too little real text|Latin words count as units; CJK text counts by character|Timestamp-to-content ratio \(0-1\)\. Higher flags timestamp noise/);
  ...
  assert.match(commonRulesForm, /For Following and AI Brief summaries/);
  assert.match(commonRulesForm, /Before source-specific fetch prompts/);
  ...
  assert.doesNotMatch(commonRulesForm, /feeds or digests|Following or AI Briefs|Used before source-specific fetch prompts when the Local Agent extracts source content|Used when fetched posts are summarized|Applies when fetched posts are summarized|Applies to summaries in Following posts and AI Briefs|best available extraction method/);
  ...
  assert.doesNotMatch(settingsPage, /Manage access keys and rules for Fetch sources and AI Brief\.|Access keys, source fetching, and AI Brief rules\.|Manage access keys, source fetching, and AI Brief rules\.|Manage Local Agent access and the rules used by Fetch sources and AI Brief\.|Manage access keys and the rules used by Fetch sources and AI Brief\.|Manage access keys and rules for Fetch sources and AI Briefs\./);
  ...
  assert.match(settingsLoading, /className="page-pad page-pad--settings settings-loading"/);
  assert.match(settingsLoading, /<PageHeader[\s\S]*title=\{<I18nText id="workspace\.settings" \/>\}[\s\S]*description=\{<I18nText id="workspace\.settingsDesc" \/>\}/);
  assert.doesNotMatch(settingsLoading, /Manage access keys and rules for Fetch sources and AI Brief\.|Access keys, source fetching, and AI Brief rules\.|Manage access keys, source fetching, and AI Brief rules\.|Manage Local Agent access and the rules used by Fetch sources and AI Brief\.|Manage access keys and the rules used by Fetch sources and AI Brief\.|Manage access keys and rules for Fetch sources and AI Briefs\./);
  assert.match(settingsLoading, /className="workspace-content-stack settings-workspace"/);
  assert.match(settingsLoading, /@\/components\/AgentTokenPanelSkeleton/);
  assert.match(settingsLoading, /<AgentTokenPanelSkeleton \/>/);
  const agentTokenPanelSkeleton = source("src/components/AgentTokenPanelSkeleton.tsx");
...
test("desktop shell uses centered top navigation and merged home feeds", () => {
  ...
  assert.doesNotMatch(dashboardPage, /Read AI Brief and Following updates, then save or open originals\./);
  ...
  assert.doesNotMatch(buildersPage, /your-digest-panel|imported-digest-panel/);
  ...
  assert.match(digestPipelineSelector, /aria-label=\{`AI Brief: \$\{selectedLabel\}`\}/);
  assert.match(digestPipelineSelector, /aria-label=\{`Choose AI Brief, current: \$\{selectedLabel\}`\}/);
  ...
  assert.doesNotMatch(digestPipelineSelector, /aria-label="AI Brief choices"|digestSourceLabel/);
  ...
  assert.doesNotMatch(digestArchivePicker, /aria-label="Choose AI Brief archive"/);
  assert.doesNotMatch(digestArchivePicker, /className="digest-picker-static" aria-label="AI Brief archive"/);
  ...
  assert.doesNotMatch(dashboardPage, /has not shared any AI Brief archives yet/);
  assert.doesNotMatch(dashboardPage, /The owner has not shared any AI Brief archives yet/);
  ...
  assert.match(dashboardPage, /Add sources for AI Brief and Following\./);
  assert.doesNotMatch(dashboardPage, /Add sources to feed AI Brief and Following\.|Add sources in the Sources tab|Add sources in Sources\. They feed AI Brief|They feed AI Brief and Following\./);
  assert.doesNotMatch(dashboardPage, /start building AI Briefs/);
  assert.match(dashboardPage, /No summarized posts yet/);
  assert.doesNotMatch(dashboardPage, /No fetched posts yet/);
  assert.match(dashboardPage, /Copy a Build AI Brief prompt to create an issue/);
  assert.match(dashboardPage, /Copy a Fetch sources prompt to summarize followed posts/);
  assert.doesNotMatch(dashboardPage, /Build an AI Brief from summarized posts|Build AI Brief from summarized posts|Run Fetch sources to summarize followed posts|before building AI Brief/);
  assert.doesNotMatch(dashboardPage, /Ask your Local Agent to build an AI Brief from the summarized posts/);
  assert.doesNotMatch(dashboardPage, /Ask your Local Agent to fetch and summarize your followed sources/);
  assert.doesNotMatch(dashboardPage, /No digest yet|No archived AI Briefs|This imported digest|This imported AI Brief has no archived AI Brief yet|build a digest from|material for a digest|start building digests|before building a digest/);
  ...
  assert.match(digestArchivePicker, /AI Brief/);
  ...
  assert.match(builderDetailPage, /return "FollowBrief source library"/);
...
test("dashboard defers heavy recommendation timeline work to a client island", () => {
  ...
  assert.doesNotMatch(favoriteList, /AI Briefs, Following posts/);
  assert.doesNotMatch(favoriteList, /Starred posts|Star posts from AI Brief/);
  assert.doesNotMatch(favoriteList, /Saved posts, newest first\.|Could not remove from Favorites\. It remains here\.|post pages|to build this queue|Use the star on any post in AI Brief, Following, Search, or a post detail page to build a focused reading queue here\./);
  ...
  assert.match(followingSection, /contentSyncStateChanged/);
  assert.match(followingSection, /window\.addEventListener\(contentSyncStateChanged/);
...
test("workspace auto-refresh covers server-side data changes without manual reloads", () => {
  ...
  assert.match(workspaceLayout, /contentSyncState\(session\.user\.id, \{ isAdmin \}\)/);
  ...
  assert.match(fetchLogPanel, /No update\. Sources were checked and no new posts needed to be synced\./);
  ...
  assert.doesNotMatch(digestLogPanel, /Last \{visibleCount\} AI Brief/);
  ...
  assert.match(fetchLogPanel, /return \{ label: "reading", tone: "warn" \}/);
  ...
  assert.match(digestLogPanel, /This AI Brief build is no longer in the current history\./);
  ...
  assert.match(digestLogPanel, /AI Brief updates/);
  assert.doesNotMatch(digestLogPanel, /aria-label="AI Brief build views"/);
  ...
  assert.match(digestLogPanel, /Show AI Brief status log/);
  assert.match(digestLogPanel, /Hide AI Brief status log/);
  assert.doesNotMatch(digestLogPanel, /aria-label="AI Brief build status graph, oldest to newest"/);
  assert.doesNotMatch(digestLogPanel, /AI Brief build at/);
  assert.match(digestLogPanel, /includes posts already used in AI Brief/);
  assert.doesNotMatch(digestLogPanel, /includes posts already used in AI Briefs|includes posts already used in AI Briefs/);
  assert.match(digestLogPanel, /Untitled AI Brief/);
  assert.match(digestLogPanel, /Prepared, no AI Brief synced/);
  assert.match(digestLogPanel, /No update/);
  assert.match(digestLogPanel, /No update\. No new eligible posts in this window\./);
  assert.match(digestLogPanel, /Previous AI Brief/);
  assert.match(digestLogPanel, /Previous AI Brief <RelativeTime value=\{run\.lastDigestAt\} \/>/);
  assert.match(digestLogPanel, /Found, AI Brief not synced yet/);
  assert.match(digestLogPanel, /Used in the AI Brief/);
  assert.match(digestLogPanel, /Run Local Agent/);
  assert.match(digestLogPanel, /Render brief JSON/);
  assert.match(digestLogPanel, /Sync to FollowBrief/);
  assert.match(digestLogPanel, /Record included posts/);
  assert.match(digestLogPanel, /No sync reported/);
  ...
  assert.match(digestLogPanel, /Copy a Build AI Brief prompt to start a Local Agent run\./);
  assert.doesNotMatch(digestLogPanel, /Use Build AI Brief to copy a Local Agent prompt\.|Use Build digest to copy a Local Agent prompt\./);
  assert.match(digestLogPanel, /className="sync-panel-column"/);
  ...
  assert.doesNotMatch(digestLogPanel, /aria-label="Digest update views"|>Digest updates<|Show digest update details|Hide digest update details|aria-label="Digest schedule status graph"|scheduled brief run|already digested items|past items|new items|not sync a digest|Untitled digest|Prepared, no digest synced|Previous digest|Found, digest not synced yet|Used in the digest/);
  ...
  assert.match(contentStateRoute, /contentSyncState\(session\.user\.id, \{ isAdmin \}\)/);
  ...
  assert.match(contentSyncState, /const inflight = contentSyncStateInflight\.get\(key\)/);
  assert.match(contentSyncState, /contentSyncStateInflight\.set\(key, nextState\)/);
  assert.match(contentSyncState, /function pruneContentSyncStateCache/);
  assert.match(contentSyncState, /builderPoolEntry\.findMany/);
  assert.match(contentSyncState, /const builderIds = poolEntries\.map/);
  ...
  assert.match(contentSyncState, /feedRead\.aggregate/);
  assert.match(contentSyncState, /recommendationSnapshot\.aggregate/);
  ...
  assert.match(contentSyncState, /libraryHubEntry\.findMany\([\s\S]*importCount: true/);
  assert.match(contentSyncState, /libraryHubItem\.findMany/);
  assert.match(contentSyncState, /digestPipelineShare\.findMany/);
  ...
  assert.match(digestLogPanel, /title="No AI Brief build history yet"/);
  assert.match(digestLogPanel, /body="Copy a Build AI Brief prompt to create history\."/);
  assert.doesNotMatch(digestLogPanel, /title="No AI Brief builds yet"|body="Started one-time and scheduled builds appear here\."|body="One-time and scheduled builds appear here after they start\."|body="One-time and scheduled AI Brief builds appear here after a Local Agent reports them\."|body="Scheduled and one-time AI Brief builds will appear here after the Local Agent reports them\."/);
...
test("skill context caps personal fetched items to keep payloads bounded", () => {
  ...
  // gated by intent so library fetches don't appear as AI Briefs.
  assert.match(contextRoute, /const isDigest =/);
  assert.match(contextRoute, /if \(isDigest && !dryRun\)/);
});

test("dashboard digest tab owns the AI Brief selector", () => {
  const dashboardPage = source("src/app/(workspace)/dashboard/page.tsx");
  const digestArchivePicker = source("src/components/DigestArchivePickerView.tsx");
  const digestPipelineSelector = source("src/components/DigestPipelineSelectorView.tsx");
  const buildersPage = source("src/app/(workspace)/builders/page.tsx");
  ...
  assert.match(dashboardPage, /aria-label="Selected AI Brief"/);
  ...
  assert.match(digestArchivePicker, /className="digest-picker-issue">Brief #\{digest\.issueNumber\}/);
  ...
  assert.match(digestArchivePicker, /className="digest-picker-static" aria-label=\{`AI Brief: \$\{selectedLabel\}`\}/);
  ...
  assert.match(dashboardPage, /AI Brief/);
  ...
  assert.doesNotMatch(dashboardPage, /digestPipelineShare\.findUnique/);
  ...
  assert.equal(existsSync(join(root, "src/components/DigestPipelineTitleEditor.tsx")), false);
  ...
  assert.match(dashboardPage, /ownerLabel: "Your AI Brief"/);
  assert.doesNotMatch(dashboardPage, /Imported AI Brief collection|ownPipelineShare/);
  ...
  assert.match(buildersPage, /ariaLabel="Sources and AI Brief tabs"/);
  assert.doesNotMatch(buildersPage, /ariaLabel="Sources and AI Brief"/);
  ...
  assert.doesNotMatch(buildersPage, /Your AI Brief archive/);
  assert.doesNotMatch(buildersPage, />\s*Your AI Brief\s*<\/h2>/);
  assert.match(buildersPage, /className="digest-source-management digest-brief-list"/);
  assert.doesNotMatch(buildersPage, /<FollowBriefDigestPipelineCard|followBriefDigestPipeline/);
  assert.doesNotMatch(buildersPage, /your-digest-panel|imported-digest-panel/);
  assert.doesNotMatch(digestPipelineForm, /FollowBrief AI Brief/);
  ...
  assert.match(digestPipelineForm, /HEADLINE_PREVIEW_LINES = 6/);
  assert.match(digestPipelineForm, /collapsedLineCount=\{HEADLINE_PREVIEW_LINES\}/);
  assert.match(digestPipelineForm, /sourceLinks=\{pipeline\.latestDigestSourceLinks\}/);
  assert.match(digestHeadlineSummary, /aria-label="AI Brief headlines"/);
  assert.equal((digestPipelineForm.match(/value=\{pipeline\.digestCount\}/g) ?? []).length, 1);
  ...
  assert.doesNotMatch(digestPipelineForm, /pipeline\.digestCount === 1 \? "AI Brief archive" : "AI Brief archives"/);
  assert.match(digestPipelineForm, /No AI Briefs yet/);
  assert.doesNotMatch(digestPipelineForm, /No AI Brief archives yet/);
  assert.doesNotMatch(digestPipelineForm, /pipeline\.digestCount === 1 \? "archive entry" : "archive entries"/);
  assert.doesNotMatch(digestPipelineForm, /archived AI Brief/);
  assert.doesNotMatch(digestPipelineForm, /saved AI Brief|saved brief/);
...
test("AI Brief storage and rendering use structured items instead of markdown content", () => {
  const schema = source("prisma/schema.prisma");
...
test("search page uses a client form with pending feedback", () => {
  ...
  assert.doesNotMatch(searchPage, /saved posts, and AI Briefs/);
  ...
  assert.match(searchPage, /AI Briefs/);
  ...
  assert.match(searchPage, /typeFilter === "digest"[\s\S]*"AI Brief"[\s\S]*"AI Briefs"/);
  ...
  assert.match(searchPage, /digest:\s*"AI Brief"/);
  assert.doesNotMatch(searchPage, /digest:\s*"AI Digest/);
  ...
  assert.doesNotMatch(searchPage, /past AI Briefs/);
  ...
  assert.doesNotMatch(searchPage, /Find sources, posts, saved posts, and AI Briefs in one place\./);
  assert.doesNotMatch(searchPage, /Find sources, posts, saved posts, and AI Briefs in one place\./);
  ...
  assert.doesNotMatch(searchForm, /placeholder="Search sources, posts, saved posts, and AI Briefs"/);
...
test("search suggestions API exists for autocomplete-style queries", () => {
  ...
  assert.doesNotMatch(suggestRoute, /AI Brief archives/);
  assert.match(suggestRoute, /AI Brief/);
  assert.doesNotMatch(suggestRoute, /past AI Briefs/);
...
test("primary tabs keep local loading fallbacks alongside route loaders", () => {
  ...
  assert.match(buildersPage, /<span className="sr-only">Loading AI Brief controls<\/span>/);
  assert.doesNotMatch(buildersPage, /Loading AI Brief settings/);
  ...
  assert.doesNotMatch(buildersPage, /Follow sources, run Fetch sources, and choose what feeds AI Briefs and Following posts\.|choose what feeds AI Briefs and Following posts\./);
  ...
  assert.doesNotMatch(libraryHubPage, /DigestPipelineImportFallback/);
  ...
  assert.doesNotMatch(libraryHubPage, /loadDigestPipelineHubPageData|WorkspaceTabShell|ai-digests/);
  ...
  assert.match(libraryHubPage, /const sourceLibraryDataPromise = loadSourceLibraryHubPageData\(\)/);
  assert.match(libraryHubPage, /function LibraryHubImportFallback/);
  assert.doesNotMatch(libraryHubPage, /tabs\.sourceLibraries|tabs\.aiDigestCollections/);
  assert.doesNotMatch(libraryHubPage, /Import shared source libraries for AI Brief and Following\./);
  assert.doesNotMatch(libraryHubPage, /Import libraries that feed AI Brief and Following\.|Import shared source libraries into Sources\.|Import shared source libraries into Sources for AI Brief and Following\.|Import shared source libraries into the Sources tab|Imported sources feed AI Briefs and Following posts\./);
  assert.doesNotMatch(libraryHubPage, /Import shared libraries into Sources\.|Import shared libraries into Sources, Following, and AI Brief\./);
  ...
  assert.doesNotMatch(libraryHubPage, /Browse shared source libraries and AI Brief collections, then import the ones you need\./);
  assert.doesNotMatch(libraryHubPage, /Hub tabs|Hub sections|tabpanel|selectedHubTabItem/);
  ...
  assert.doesNotMatch(libraryHubPage, /Import shared source libraries and AI Brief archives/);
  ...
  assert.doesNotMatch(libraryHubPage, /label=\{data\.digestPipelineCount === 1 \? "AI Brief" : "AI Briefs"\}/);
  ...
  assert.doesNotMatch(libraryHubPage, /Loading shared AI Brief collections/);
  assert.doesNotMatch(libraryHubPage, /Import shared source libraries for AI Brief and Following\./);
  assert.doesNotMatch(libraryHubPage, /Import libraries that feed AI Brief and Following\.|Import shared source libraries into Sources\.|Import shared libraries into Sources\.|Import shared libraries into Sources, Following, and AI Brief\./);
  assert.doesNotMatch(libraryHubPage, /Shared AI Brief collections|AI Brief archives|Import AI Brief collections/);
  assert.doesNotMatch(libraryHubPage, /AI Brief archives shared to Hub\./);
  ...
  assert.doesNotMatch(libraryHubPage, /className="library-hub-skeleton-line is-wide"/);
  ...
  assert.doesNotMatch(libraryHubPage, /className="library-hub-skeleton-pill"/);
  assert.doesNotMatch(libraryHubPage, /library-hub-skeleton-digest-preview|library-hub-skeleton-meta-grid/);
  assert.doesNotMatch(libraryHubPage, /<div className="fb-hub-card library-hub-skeleton-card" \/>/);
  assert.doesNotMatch(libraryHubPage, /<h2 className="fb-section-heading">Source libraries<\/h2>/);
  ...
  assert.doesNotMatch(source("src/components/LibraryHubImportForm.tsx"), /className="library-hub-toolbar"/);
  assert.doesNotMatch(source("src/components/LibraryHubImportForm.tsx"), /className="library-hub-toolbar-copy"/);
  assert.doesNotMatch(source("src/components/LibraryHubImportForm.tsx"), /<h2 className="fb-section-heading">Source libraries<\/h2>/);
  assert.doesNotMatch(source("src/components/LibraryHubImportForm.tsx"), /Import shared source libraries for AI Brief and Following\./);
  assert.doesNotMatch(source("src/components/LibraryHubImportForm.tsx"), /Import libraries that feed AI Brief and Following\.|Import shared source libraries into Sources\.|Import shared source libraries into Sources for AI Brief and Following\.|Import shared source libraries into the Sources tab|Imported sources feed AI Briefs and Following posts\./);
  assert.doesNotMatch(source("src/components/LibraryHubImportForm.tsx"), /Import shared libraries into Sources\.|Import shared libraries into Sources, Following, and AI Brief\./);
  ...
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /Source libraries selected and maintained by FollowBrief\./);
  ...
  assert.doesNotMatch(source("src/components/LibraryHubImportForm.tsx"), /Imported libraries available in Sources, Following, and AI Brief\./);
  ...
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /label:\s*"FollowBrief"/);
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /shortLabel:\s*"FollowBrief"/);
  ...
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /if \(library\.isCommunity\) \{[\s\S]*<FollowBriefLibraryIdentity compact label="FollowBrief"/);
  ...
  assert.match(digestPipelineForm, /function DigestPipelineInfoCard/);
  assert.equal((digestPipelineForm.match(/<DigestPipelineInfoCard/g) ?? []).length, 1);
  ...
  assert.doesNotMatch(digestPipelineForm, /followbrief-digest-card/);
  assert.match(digestPipelineForm, /<article className=\{className\}>/);
  assert.doesNotMatch(digestPipelineForm, /DigestPipelineTitleEditor|DigestPipelineVisibilityToggle|sharedPipelines|importedPipelines/);
  ...
  assert.match(digestPipelineForm, /aria-label="AI Brief details"/);
  ...
  assert.doesNotMatch(digestPipelineForm, /Latest AI Brief/);
  assert.doesNotMatch(digestPipelineForm, /\?\s*`Latest AI Brief \$\{formatDate\(pipeline\.latestDigestAt\)\}`/);
  ...
  assert.match(source("src/components/DigestPipelineImportForm.tsx"), /className="own-digest-card"/);
  assert.doesNotMatch(source("src/components/DigestPipelineImportForm.tsx"), /followbrief-digest-card/);
  assert.doesNotMatch(source("src/components/DigestPipelineImportForm.tsx"), /className="hub-card-action-row"/);
  assert.doesNotMatch(source("src/components/DigestPipelineImportForm.tsx"), /Import AI Brief collection/);
  assert.doesNotMatch(source("src/components/DigestPipelineImportForm.tsx"), /\?\s*"Importing"\s*:\s*"Import"/);
  assert.doesNotMatch(source("src/components/DigestPipelineImportForm.tsx"), /"Import AI Brief archive"|Importing AI Brief archive|Import archive/);
  assert.doesNotMatch(source("src/components/DigestPipelineImportForm.tsx"), /handleRemoveDialogClose|hub-card-action-button|hub-list-empty|EmptyState/);
...
test("builders page exposes per-builder fetched posts ordered by time", () => {
  ...
  const feedItemsRouteHelper = source("src/lib/builder-feed-items-route.ts");
  ...
  assert.match(feedItemsRouteHelper, /Source is not in your source library\./);
  assert.doesNotMatch(feedItemsRouteHelper, /Source is not in your library/);
  ...
  assert.doesNotMatch(buildersPage, /Add sources, then run Fetch sources\.|Add sources, then run Fetch sources for AI Brief and Following\.|Add sources, then run Fetch sources\. They feed AI Brief|AI Brief and Following use this library|They feed AI Brief and Following/);
  ...
  assert.match(postDetailPage, /returnTo\.includes\("tab=following"\) \? "Following" : "AI Brief"/);
  assert.match(postDetailPage, /case "Sources"/);
  assert.match(postDetailPage, /case "Source":[\s\S]*return "Sources"/);
  assert.match(postDetailPage, /case "AI Brief"/);
  ...
  assert.match(feedItemsRoute, /createBuilderFeedItemsGetHandler/);
  assert.match(feedItemsRouteHelper, /fetchDedupedFeedForEntities/);
  assert.match(feedItemsRouteHelper, /activePoolBuilderIds/);
  assert.match(feedItemsRouteHelper, /prisma\.libraryHubItem\.findFirst/);
  assert.match(feedItemsRouteHelper, /resolveBuilderFeedAccess/);
  assert.match(feedItemsRouteHelper, /NextResponse\.json/);
  assert.match(feedItemsRouteHelper, /Could not load summarized posts\./);
  assert.doesNotMatch(feedItemsRouteHelper, /Failed to load summarized posts/);
  ...
  assert.match(digestContent, /detailUrl:\s*postDetailHref\(item\.post\.feedItemId, "\/dashboard\?tab=ai-digest", "AI Brief"\)/);
...
test("library hub exposes share and multi-import flows", () => {
  ...
  assert.match(visibilityToggle, /FollowBrief source library on Hub/);
  ...
  assert.doesNotMatch(hubPage, /sharePersonalLibraryToHubAction|importHubLibrariesAction/);
  ...
  assert.match(hubPage, /loadSourceLibraryHubPageData[\s\S]*ensureDefaultCommunityLibraryImport\(session\.user\.id\)[\s\S]*prisma\.libraryHubEntry\.findMany/);
  assert.match(hubPage, /LibraryHubImportForm/);
  assert.doesNotMatch(hubPage, /DigestPipelineImportForm|digestPipelineShare|digestPipelineImport|ai-digests|WorkspaceTabShell/);
  assert.match(hubPage, /adminCommunityLibraryName/);
  assert.match(source("src/lib/library-hub.ts"), /adminCommunityLibraryName = followBriefSourceLibraryName/);
  assert.match(source("src/lib/followbrief-library.ts"), /followBriefSourceLibraryName = "FollowBrief source library"/);
  assert.match(source("src/lib/followbrief-library.ts"), /Sources selected and maintained by FollowBrief\./);
  ...
  assert.match(source("src/lib/builder-pool.ts"), /adminCommunityLibraryName = followBriefSourceLibraryName/);
  ...
  assert.doesNotMatch(buildersPage, /ownPipelineShare|ensureDefaultCommunityDigestImport|DigestPipelineTitleEditor|DigestPipelineVisibilityToggle/);
  assert.match(hubImportForm, /"use client"/);
  assert.match(hubImportForm, /import \{ FollowBriefLibraryIdentity \} from "@\/components\/FollowBriefLibraryIdentity"/);
  assert.match(hubImportForm, /library\.isCommunity[\s\S]*<FollowBriefLibraryIdentity/);
  assert.match(hubImportForm, /className="followbrief-library-title"/);
  assert.match(hubImportForm, /className="followbrief-library-owner"/);
  assert.match(hubImportForm, /<UserName>/);
  assert.match(source("src/components/FollowBriefLibraryIdentity.tsx"), /<BrandMark className="followbrief-library-mark"/);
  assert.match(buildersPage, /isFollowBrief:\s*isCommunityLibrary/);
  assert.match(buildersPage, /library\.isFollowBrief[\s\S]*<FollowBriefLibraryIdentity/);
  assert.match(globals, /\.followbrief-library-identity/);
  assert.match(globals, /\.followbrief-library-mark\.fb-mark/);
  assert.doesNotMatch(hubImportForm, /No community source libraries match this filter\./);
  assert.doesNotMatch(hubImportForm, /label: "Community"|shortLabel: "Community"/);
  ...
  assert.match(hubImportForm, /removes its sources\s+from Sources and stops feeding AI Brief and Following\./);
  assert.doesNotMatch(hubImportForm, /Sources tab|AI Briefs and Following posts|sources\s+from this library will no longer appear/);
  ...
  assert.match(libraryImportRemoveButton, /removes its sources from\s+Sources and stops feeding AI Brief and Following\./);
  assert.doesNotMatch(libraryImportRemoveButton, /Sources tab|AI Briefs and Following posts|sources from this\s+library will no longer appear/);
  ...
  assert.equal(existsSync(join(root, "src/components/DigestPipelineVisibilityToggle.tsx")), false);
  assert.equal(existsSync(join(root, "src/components/DigestPipelineTitleEditor.tsx")), false);
  assert.match(source("src/lib/library-hub.ts"), /adminCommunityDigestTitle = "FollowBrief AI Brief"/);
  assert.match(source("src/lib/library-hub.ts"), /"The AI Brief curated by FollowBrief\."/);
  assert.match(digestPipelineShareRoute, /status: 404/);
  assert.match(digestPipelineImportRoute, /status: 404/);
  assert.match(digestPipelineRemoveRoute, /status: 404/);
  assert.doesNotMatch(digestPipelineShareRoute, /shareDigestPipelineToHub|updateDigestPipelineTitle|unshareDigestPipelineFromHub/);
  assert.doesNotMatch(digestPipelineImportRoute, /importDigestPipelineFromHub/);
  assert.doesNotMatch(digestPipelineRemoveRoute, /removeDigestPipelineImportFromHub/);
  ...
  assert.match(source("src/lib/library-hub.ts"), /return "Your AI Brief"/);
  assert.match(source("src/lib/library-hub.ts"), /replace\(\/AI Builder Digest\/g, "AI Brief"\)/);
  assert.match(source("src/lib/library-hub.ts"), /replace\(\/Builder Digest\/g, "AI Brief"\)/);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.doesNotMatch(settingsPage, /Rules for fetching and summarizing source posts\.|Runs before AI Brief: discover source posts/);
  ...
  assert.doesNotMatch(settingsPage, /USER_DIGEST_PROMPT_COUNT|digestPromptCount/);
  assert.match(settingsPage, /canEditDigestAssemblyPrompts=\{isAdmin\}/);
  assert.match(settingsPage, /Build AI Briefs\./);
  assert.doesNotMatch(settingsPage, /Rules for AI Brief builds\.|Rules for building AI Briefs\./);
  assert.doesNotMatch(settingsPage, /Runs after per-post summaries: write the digest headline, source notes,\s*and post summaries in the selected AI Brief language\./);
  assert.doesNotMatch(settingsPage, /Write AI Brief headlines, source notes, and translated post summaries\.|Used when building AI Briefs\.|Used when Fetch sources reads and summarizes posts\./);
  ...
  assert.doesNotMatch(settingsPage, /Write AI Brief headlines, source notes, and localized post summaries\./);
  ...
  assert.match(settingsPage, /<Suspense fallback=\{<SettingsRulesSkeleton showDigestRules=\{isAdmin\} \/>/);
  ...
  const settingsRulesSkeleton = source("src/components/SettingsRulesSkeleton.tsx");
  assert.match(settingsRulesSkeleton, /showDigestRules/);
  assert.match(settingsRulesSkeleton, /\? \["Source fetching rules", "AI Brief rules"\]/);
  assert.match(settingsRulesSkeleton, /: \["Source fetching rules"\]/);
  ...
  assert.doesNotMatch(tokenPanel, />\s*Local Agents that can sync sources and AI Briefs to this account\.|Devices and Local Agents that can sync sources and AI Briefs to this account\./);
  assert.doesNotMatch(tokenPanel, /Access keys authorize Local Agent runs from your devices\.|Access keys let Local Agent update this account from your devices\.|Access keys let devices and Local Agents update this account\.|Access keys let trusted devices and Local Agents sync sources and AI Briefs to this account\./);
