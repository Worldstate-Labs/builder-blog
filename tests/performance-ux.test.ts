...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.doesNotMatch(commonRulesForm, /Used before source-specific Local Agent fetch prompts/);
  ...
  assert.match(adminSourceTypeManager, /Adds source-specific extraction instructions\./);
  ...
  assert.match(adminSourceTypeManager, /Filters extracted content before Following or AI Digest/);
  ...
  assert.match(commonRulesForm, /Applies to Following and AI Digest summaries/);
  assert.match(commonRulesForm, /Runs before source-specific Local Agent fetch prompts/);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(tokenPanel, /Create one before connecting a device or Local Agent\./);
  ...
  assert.match(tokenPanel, /This key has never been used/);
  assert.match(tokenPanel, /It needs a new access key to update\s+FollowBrief/);
  ...
  assert.doesNotMatch(tokenPanel, /That device or Local Agent needs a new access key|After revoking it, that device or Local Agent will need|will need a new access key|Revoking it now is safe|that Local Agent will lose access|will lose access|No machine will lose access/);
