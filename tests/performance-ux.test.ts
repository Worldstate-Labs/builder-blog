...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-height:\s*2\.55rem/);
...
test("search page uses a client form with pending feedback", () => {
  ...
  assert.match(searchPage, /emptySearchCopyByType: Record<SearchTypeFilter, string>/);
  assert.match(searchPage, /all: "Search sources, posts, and AI Digest archives\."/);
  assert.match(searchPage, /builder: "Search sources\."/);
  assert.match(searchPage, /feed: "Search posts\."/);
  assert.match(searchPage, /digest: "Search AI Digest archives\."/);
  assert.match(searchPage, /function emptySearchCopyForType\(typeFilter: SearchTypeFilter\)/);
  assert.match(searchPage, /\{emptySearchCopyForType\(typeFilter\)\}/);
  assert.match(searchPage, /\{emptySearchCopyForType\(current\)\}/);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-height:\s*2\.55rem/);
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-width:\s*10rem/);
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*text-align:\s*center/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-card\s*{[\s\S]*grid-template-columns:\s*2\.5rem minmax\(0,\s*1fr\)/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-card\s*{[\s\S]*min-height:\s*5\.5rem/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*grid-column:\s*2/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-height:\s*2\.4rem/);
