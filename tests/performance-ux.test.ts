...
test("public entry pages use the centered product layout", () => {
  ...
  assert.match(landingPage, /Read their updates as one AI Digest issue\./);
  assert.doesNotMatch(landingPage, /Read them as one AI Digest|Read their updates as one AI Digest\./);
  ...
  assert.match(landingPage, /Find sources, posts, and AI Digest issues/);
  ...
  assert.match(landingPage, /Open originals and search sources, posts, and AI Digest issues\./);
  assert.doesNotMatch(landingPage, /Open original posts and search sources, posts, and AI Digest issues later|search sources, posts, and AI Digest issues later/);
  ...
  assert.match(landingPage, /Read their updates as one AI Digest issue\./);
  assert.doesNotMatch(landingPage, /Read them as one AI Digest issue\./);
  ...
  assert.match(landingPage, /build a cited AI Digest issue/);
  assert.doesNotMatch(landingPage, /build a cited AI Digest from the summaries|build a cited AI Digest\./);
  ...
  assert.match(landingPage, /Unread posts stay separate from AI Digest issues\./);
  assert.doesNotMatch(landingPage, /Unread posts from followed sources stay separate from AI Digest issues\.|Unread posts from followed sources stay separate from the AI Digest\./);
  ...
  assert.match(landingPage, /Follow sources, build AI Digest, and search later\./);
  assert.match(landingPage, /Search and revisit/);
  assert.match(landingPage, /Search when needed/);
  assert.doesNotMatch(landingPage, /Search after reading|Search after the read\./);
...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-card\s*{[\s\S]*grid-template-columns:\s*1\.75rem minmax\(0,\s*1fr\)/);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-card\s*{[\s\S]*min-height:\s*4\.5rem/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*grid-column:\s*2/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-height:\s*2\.05rem/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-width:\s*7rem/);
