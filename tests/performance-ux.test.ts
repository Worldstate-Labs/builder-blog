...
test("every app route has an explicit centered layout role", () => {
  ...
  assert.match(searchLoading, /label:\s*"AI Digest",\s*selected:\s*false/);
  ...
  assert.match(searchLoading, /search-loading-tab[\s\S]*AI Digest/);
...
test("search page uses a client form with pending feedback", () => {
  ...
  assert.match(searchPage, /label:\s*"Best match"/);
  assert.match(searchPage, /label:\s*"All results"/);
  assert.match(searchPage, /Any time/);
...
test("primary tabs keep local loading fallbacks alongside route loaders", () => {
  ...
  assert.match(libraryHubPage, /Import shared libraries into Sources\./);
  ...
  assert.match(libraryHubPage, /Import shared collections\./);
  assert.doesNotMatch(libraryHubPage, /AI Digest tab|Import shared AI Digest collections into AI Digest\./);
  ...
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /Import shared libraries into Sources\./);
...
test("builders page exposes per-builder fetched posts ordered by time", () => {
  ...
  assert.match(personalBuilderRoute, /Source added unverified\. Local Agent can retry later\./);
  ...
  assert.match(personalBuilderUpdateRoute, /Source updated unverified\. Local Agent can retry later\./);
  ...
  assert.match(postCard, /Full content from Fetch sources\./);
  assert.doesNotMatch(postCard, /Full content from Fetch sources, collapsed until needed\./);
...
test("library hub exposes share and multi-import flows", () => {
  ...
  assert.match(digestPipelineForm, /Import shared collections\./);
  assert.match(digestPipelineForm, /Imported collections appear in AI Digest\./);
  assert.doesNotMatch(digestPipelineForm, /AI Digest tab|Import shared AI Digest collections into AI Digest\.|Available in AI Digest\.|Already in AI Digest\.|Collections already in AI Digest\./);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(tokenPanel, /Create one for a device or Local Agent\./);
  assert.doesNotMatch(tokenPanel, /No Local Agent access yet/);
  assert.doesNotMatch(tokenPanel, /Add one when you connect a Local Agent\.|Add an access key when connecting a device or Local Agent\.|Add an access key when you connect a device or Local Agent\./);
