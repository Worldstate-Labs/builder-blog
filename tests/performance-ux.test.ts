...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(globals, /\.access-key-device-title\s*{[\s\S]*font-size:\s*1\.125rem/);
  assert.match(globals, /\.access-key-device-status\s*{[\s\S]*font-size:\s*0\.9375rem/);
  assert.match(globals, /\.access-key-device-icon svg\s*{[\s\S]*height:\s*1\.35rem/);
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*justify-self:\s*end/);
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-height:\s*2\.5rem/);
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-width:\s*10\.25rem/);
