...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(commonRulesForm, /source-specific Local Agent fetch prompts/);
  ...
  assert.doesNotMatch(adminSourceTypeManager, /How each post from this source is written as a per-post summary|one-time or scheduled run prompt|Output language comes from the run prompt/);
  assert.match(adminSourceTypeManager, /Extra instructions for Local Agent extraction\./);
  ...
  assert.match(adminSourceTypeManager, /Failed posts are not used in Following or AI Digest/);
  assert.match(adminSourceTypeManager, /Drop posts below this body length/);
  ...
  assert.doesNotMatch(adminSourceTypeManager, /Posts that fail are not saved or used in Following or AI Digest|Drop posts whose body has fewer characters than this|Latin words count as units; CJK text counts by character/);
  ...
  assert.match(commonRulesForm, /Used before source-specific Local Agent fetch prompts/);
  ...
  assert.doesNotMatch(commonRulesForm, /feeds or digests|Following or AI Digests|Used before source-specific fetch prompts when the Local Agent extracts source content|best available extraction method/);
...
test("primary tabs keep local loading fallbacks alongside route loaders", () => {
  ...
  assert.match(source("src/components/LibraryHubImportForm.tsx"), /Source libraries already imported from Hub\./);
...
test("library hub exposes share and multi-import flows", () => {
  ...
  assert.match(digestPipelineForm, /AI Digest archives already imported from Hub\./);
  ...
  assert.match(digestPipelineVisibilityToggle, /Remove AI Digest archive from Hub/);
  ...
  assert.match(digestPipelineForm, /Import AI Digest archives to see them in AI Digest\./);
  ...
  assert.match(digestPipelineForm, /Shared AI Digest archives appear here when users share them to Hub\./);
