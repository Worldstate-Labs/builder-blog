...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-height:\s*2\.2rem/);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-keys-list--skeleton \.settings-skeleton-row\s*{[\s\S]*height:\s*4\.75rem/);
  ...
  assert.match(globals, /\.access-key-device-title\s*{[\s\S]*font-size:\s*1\.0625rem/);
  assert.match(globals, /\.access-key-device-status\s*{[\s\S]*font-size:\s*0\.9375rem/);
  assert.match(globals, /\.access-key-device-icon svg\s*{[\s\S]*height:\s*1\.375rem/);
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*justify-self:\s*end/);
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-height:\s*2\.2rem/);
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-width:\s*8\.75rem/);
