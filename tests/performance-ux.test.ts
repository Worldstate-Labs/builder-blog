...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(globals, /\.access-key-card\s*{[\s\S]*grid-template-columns:\s*2\.75rem minmax\(0,\s*1fr\) auto/);
  assert.match(globals, /\.access-key-card\s*{[\s\S]*min-height:\s*7\.25rem/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-card\s*{[\s\S]*grid-template-columns:\s*2\.75rem minmax\(0,\s*1fr\)/);
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-card\s*{[\s\S]*min-height:\s*6\.75rem/);
  ...
  assert.match(globals, /\.access-key-device-title\s*{[\s\S]*font-size:\s*1\.2rem/);
  ...
  assert.match(globals, /\.access-key-revoke-button,[\s\S]*\.access-key-revoked-pill\s*{[\s\S]*min-height:\s*2\.7rem/);
...
test("desktop shell uses centered top navigation and merged home feeds", () => {
  ...
  assert.match(digestArchivePicker, /event\.key !== "ArrowDown" && event\.key !== "ArrowUp"/);
  assert.match(digestArchivePicker, /window\.requestAnimationFrame\(\(\) => focusOption\("selected"\)\)/);
  assert.match(digestArchivePicker, /focusOption\(event\.key === "ArrowDown" \? "next" : "previous"\)/);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-keys-list--skeleton \.settings-skeleton-row\s*{[\s\S]*height:\s*6\.75rem/);
  ...
  assert.match(globals, /\.access-key-card\s*{[\s\S]*grid-template-columns:\s*2\.75rem minmax\(0,\s*1fr\) auto/);
  assert.match(globals, /\.access-key-card\s*{[\s\S]*border-radius:\s*18px/);
  assert.match(globals, /\.access-key-card\s*{[\s\S]*background:\s*color-mix\(in oklch, var\(--paper\) 92%, var\(--paper-strong\)\)/);
  assert.match(globals, /\.access-key-card\s*{[\s\S]*box-shadow:\s*none/);
  assert.match(globals, /\.access-key-card\s*{[\s\S]*min-height:\s*7\.25rem/);
  assert.match(globals, /\.access-key-device-title\s*{[\s\S]*font-size:\s*1\.2rem/);
  assert.match(globals, /\.access-key-device-status\s*{[\s\S]*font-size:\s*1\.05rem/);
  assert.match(globals, /\.access-key-device-icon svg\s*{[\s\S]*height:\s*1\.8rem/);
