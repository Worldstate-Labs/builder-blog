...
test("settings live in the clickable user avatar menu", () => {
  ...
  assert.match(globals, /\.access-key-card\s*{[\s\S]*grid-template-columns:\s*2\.35rem minmax\(0,\s*1fr\) max-content/);
  ...
  assert.match(globals, /@media \(max-width:\s*767px\)[\s\S]*\.access-key-card\s*{[\s\S]*grid-template-columns:\s*2\.35rem minmax\(0,\s*1fr\)/);
