...
test("search page uses a client form with pending feedback", () => {
  ...
  assert.match(searchPage, /emptySearchCopy = "Search sources, posts, and AI Digest archives\."/);
  assert.doesNotMatch(searchPage, /emptySearchCopy = "Search sources, posts, saved posts, and AI Digest archives\."/);
...
test("settings mutations stay local instead of refreshing the whole route", () => {
  ...
  assert.match(tokenPanel, /No active access/);
  assert.match(tokenPanel, /Revoked access is hidden from this list\./);
