...
test("every app route has an explicit centered layout role", () => {
  ...
  assert.match(searchLoading, /<PageHeader[\s\S]*title="Search"[\s\S]*Find sources, posts, and AI Digest issues in one place\./);
...
test("workspace not-found uses the FollowBrief shell instead of the default Next 404", () => {
  ...
  assert.doesNotMatch(notFoundPage, />\s*Search\s*<\/Link>/);
...
test("public entry pages use the centered product layout", () => {
  ...
  assert.match(landingPage, /Open original posts and search sources, posts, and AI Digest issues later/);
  assert.match(landingPage, /search sources, posts, and AI Digest issues later/);
  ...
  assert.match(loginPage, /Follow sources, read cited AI Digest issues/);
  assert.match(loginPage, /search them\s*alongside sources and posts later/);
  assert.doesNotMatch(loginPage, /Follow sources, read the cited AI Digest|Follow source libraries|Follow source libraries, read AI Digests|read cited AI Digests/);
  ...
  assert.match(loginPage, /After signing in, add access keys in Settings\./);
  assert.doesNotMatch(loginPage, /After sign-in|Access keys are set up after sign-in from Settings\./);
  assert.doesNotMatch(loginPage, /label="Local Agent"/);
  assert.doesNotMatch(loginPage, /KeyRound/);
  assert.match(rootLayout, /Follow sources, read cited AI Digest issues, and search them alongside sources and posts\./);
  assert.doesNotMatch(rootLayout, /Follow source libraries/);
  assert.doesNotMatch(rootLayout, /read the cited AI Digest|read cited AI Digests/);
...
test("search page uses a client form with pending feedback", () => {
  ...
  assert.match(searchPage, /<PageHeader[\s\S]*title="Search"[\s\S]*Find sources, posts, and AI Digest issues in one place\./);
