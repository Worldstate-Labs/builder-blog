...
test("skill fetch-runs route validates payload size and gates auth on user or bearer", () => {
  ...
  assert.match(route, /countPlannedPostTasks/);
  assert.match(route, /tasksGenerated: plannedPosts/);
...
test("agent runner tags cron-driven CLI runs as source=cron", () => {
  ...
  assert.doesNotMatch(runner, /run_with_hermes|agent_output_file hermes|capture_runtime_usage hermes/);
...
test("FetchLogPanel renders status pills and modal-only logs with semantic CSS variables", () => {
  ...
  assert.match(panel, /isHiddenFailureReason\(reason\)/);
  assert.match(panel, /const reason = isInternalJobRunReason\(details\.reason\) \? null : fetchFailureMessage\(details\.reason\)/);
  ...
  assert.match(panel, /<RelativeTime value=\{latestRun\?\.finishedAt \?\? latestRun\?\.startedAt\} fallback="None yet" \/>/);
  ...
  assert.doesNotMatch(panel, /<dt>Schedule stopped<\/dt>/);
  assert.doesNotMatch(panel, /<dt>Runner<\/dt>/);
  ...
  assert.doesNotMatch(panel, /className="sync-panel-stopped-time"/);
...
test("DigestLogPanel renders brief status with modal-only build logs", () => {
  ...
  assert.doesNotMatch(panel, /className="sync-panel-truncate"/);
  assert.doesNotMatch(panel, /className="sync-panel-status-brief"/);
  assert.match(panel, /className="sync-panel-layout is-log-only"/);
  assert.doesNotMatch(panel, /<dt>Schedule enabled<\/dt>/);
  assert.doesNotMatch(panel, /<dt>Next scheduled run<\/dt>/);
...
test("builders page mounts the fetch log inside the sync header section", () => {
  ...
  assert.match(buildersPage, /function hasStoppedLocalScheduleActivity/);
  assert.match(buildersPage, /cronJob\.stoppedAt \?\? cronJob\.updatedAt/);
  assert.match(buildersPage, /cronRuns\.some\(\(run\) => occurredAfter\(run\.startedAt, stoppedAtMs\)\)/);
  assert.match(buildersPage, /scheduledJobRuns\.some\(\(jobRun\) => occurredAfter\(jobRun\.startedAt, stoppedAtMs\)\)/);
