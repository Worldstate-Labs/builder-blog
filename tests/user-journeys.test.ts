...
test("web app serves the agent skill and setup command", () => {
  ...
  assert.match(skillPromptRenderer, /built-in \`cron\` tool/);
  assert.match(skillPromptRenderer, /action\\": \\"add/);
  assert.match(skillPromptRenderer, /CRON_JOB_PATH/);
  assert.match(skillPromptRenderer, /OPENCLAW_CRON_JOB_JSON/);
  assert.match(skillPromptRenderer, /sessionTarget: \\"current\\"/);
  assert.match(skillPromptRenderer, /kind: \\"agentTurn\\"/);
  assert.match(skillPromptRenderer, /timeoutSeconds/);
  assert.match(skillPromptRenderer, /bestEffort: true/);
  assert.match(skillPromptRenderer, /FOLLOWBRIEF_OPENCLAW_CRON_JOB_READY=1/);
  assert.doesNotMatch(skillPromptRenderer, /openclaw cron add/);
  assert.doesNotMatch(skillPromptRenderer, /OPENCLAW_CHANNEL_CONTEXT/);
  assert.doesNotMatch(skillPromptRenderer, /openclaw sessions --json/);
  assert.doesNotMatch(skillPromptRenderer, /OPENCLAW_PARENT_SESSION_KEY/);
  ...
  assert.doesNotMatch(skillPromptRenderer, /unattended and must not wait for confirmation/);
  ...
  assert.match(runner, /jobDefaultSeconds/);
  assert.match(runner, /library-once\|digest-once\)[\s\S]*printf '%s\\n' "43200"/);
  assert.doesNotMatch(runner, /library-once\)[\s\S]*_timeout_job="library-cron"/);
  assert.doesNotMatch(runner, /digest-once\)[\s\S]*_timeout_job="digest-cron"/);
