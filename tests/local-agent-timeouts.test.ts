...
test("local agent timeout policy is shared and clamps expected cron windows", () => {
  assert.equal(timeoutPolicy.jobDefaultSeconds["cloud-library-cron"], 22_500);
  ...
  assert.equal(localAgentTimeoutSeconds(60, "cloud-library-cron"), "22500");
  assert.equal(localAgentTimeoutSeconds(1440, "cloud-library-cron"), "22500");
  assert.equal(localAgentTimeoutSeconds(0, "cloud-library-cron"), "22500");
