...
function FollowingUnavailable({
  ...
  if (sourceReadiness.summarizedPostCount === 0) {
    return (
      <FeedEmptyState
        ...
        ariaLive="polite"
        body="Run Fetch sources to summarize posts from followed sources. Following shows the latest unread posts."
        className="is-actionable"
  ...
  return (
    <FeedEmptyState
      actions={<FetchSourcesPrompt sourceReadiness={sourceReadiness} />}
      ariaLive="polite"
      body="Following updates when Fetch sources finds new unread posts."
      className="is-actionable"
