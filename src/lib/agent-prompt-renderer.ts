...
function buildOpenClawInitialRunBootstrap({
  ...
  return [
    "Next: Queue the OpenClaw initial run and schedule install with OpenClaw's built-in `cron` tool.",
    "Run the preparation block after the setup checks pass and before any manual schedule install.",
    "The block only downloads the continuation prompt and prints the exact one-shot job object.",
    "It does not create a scheduled job.",
    "",
    "```bash",
    ...
    "CRON_JOB_PATH=\"$SETUP_TMP_DIR/openclaw-cron-job.json\"",
    ...
    "  });",
    "NODE",
    "node - \"$PROMPT_COPY\" \"$CRON_JOB_PATH\" \"$SETUP_ID\" \"$RUN_AT\" \"$OPENCLAW_SETUP_TIMEOUT_SECONDS\" <<'NODE'",
    "const { writeFileSync } = require(\"node:fs\");",
    "const [promptPath, cronJobPath, name, runAt, timeoutRaw] = process.argv.slice(2);",
    "const timeoutSeconds = Number(timeoutRaw);",
    "if (!Number.isSafeInteger(timeoutSeconds) || timeoutSeconds <= 0) {",
    "  console.error(\"OpenClaw setup timeout must be a positive integer.\");",
    "  process.exit(1);",
    "}",
    "const job = {",
    "  name,",
    ...
    "  payload: {",
    "    kind: \"agentTurn\",",
    "    message: \`Read and follow the instructions in ${promptPath} exactly.\`,",
    "    timeoutSeconds,",
    "  },",
    "  delivery: { mode: \"announce\", bestEffort: true },",
    "};",
    "writeFileSync(cronJobPath, JSON.stringify(job, null, 2) + \"\\n\");",
    "console.log(\`FOLLOWBRIEF_OPENCLAW_CRON_JOB_READY=1 path=${cronJobPath} name=${name} run_at=${runAt}\`);",
    "console.log(\"OPENCLAW_CRON_JOB_JSON=\" + JSON.stringify(job));",
    "NODE",
    "```",
    "",
    "After the block prints `FOLLOWBRIEF_OPENCLAW_CRON_JOB_READY=1`, call OpenClaw's built-in",
    "`cron` tool exactly once with top-level argument `\"action\": \"add\"` and `\"job\"`",
    "equal to the exact JSON object printed after `OPENCLAW_CRON_JOB_JSON=`.",
    "",
    "Use the built-in tool directly; do not use a shell command to create the job. The tool",
    "automatically binds `sessionTarget: \"current\"` to this OpenClaw session and its delivery",
    "route. Do not add or infer a session key, agent id, channel, recipient, or thread.",
    "",
    "If the cron tool fails, stop and report its exact error. If it succeeds, report",
    "`FOLLOWBRIEF_OPENCLAW_QUEUED=1` with the returned job id, prepared name, and run time,",
    "then stop this turn. Do not run the initial fetch or install the recurring schedule in",
    "this parent turn; the queued continuation owns those steps.",
    "",
  ].join("\n");
}
