...
function buildOpenClawInitialRunBootstrap({
  ...
  return [
    ...
    "OPENCLAW_SESSIONS_OUTPUT=\"$SETUP_TMP_DIR/openclaw-sessions.json\"",
    ...
    "  return causeMessage ? `${causeCode}${causeMessage} <- ${String(error?.message || error)}` : String(error?.message || error);",
    "}",
    "(async () => {",
    ...
    "          noRetry: response.status !== 408 && response.status !== 429 && response.status < 500,",
    "        });",
    "        throw error;",
    "      }",
    ...
    "        throw new Error(",
    "          `${formatErrorCause(error)}; attempt ${attempt}/${maxAttempts}; elapsed ${Date.now() - startedAt}ms`,",
    "          { cause: error },",
    "        );",
    "      }",
    ...
    "fi",
    "openclaw_persistent_session_mode() {",
    "  _ocpsm_help=\"$1\"",
    "  _ocpsm_session_help=\"$(printf '%s\\n' \"$_ocpsm_help\" | awk '/--session([[:space:]=]|$)/ { print; if (getline) print; exit }')\"",
    "  if printf '%s\\n' \"$_ocpsm_session_help\" | grep -Eqi '(^|[^[:alnum:]_])current([^[:alnum:]_]|$)'; then",
    "    printf 'current\\n'",
    "    return 0",
    "  fi",
    "  if printf '%s\\n' \"$_ocpsm_session_help\" | grep -Eqi '(^|[^[:alnum:]_])main([^[:alnum:]_]|$)' \\",
    "    && printf '%s\\n' \"$_ocpsm_session_help\" | grep -Eqi '(^|[^[:alnum:]_])isolated([^[:alnum:]_]|$)' \\",
    "    && printf '%s\\n' \"$_ocpsm_help\" | grep -q -- '--system-event' \\",
    "    && printf '%s\\n' \"$_ocpsm_help\" | grep -q -- '--session-key'; then",
    "    printf 'main-event\\n'",
    "    return 0",
    "  fi",
    "  return 1",
    ...
    "  exit 1",
    "fi",
    "if ! OPENCLAW_SESSION_MODE=\"$(openclaw_persistent_session_mode \"$OPENCLAW_CRON_ADD_HELP\")\"; then",
    "  echo \"OpenClaw does not expose a persistent cron session mode required for confirmation.\" >&2",
    "  exit 1",
    "fi",
    "CRON_ADD_OK=0",
    "case \"$OPENCLAW_SESSION_MODE\" in",
    "current)",
    ...
    "  ;;",
    "main-event)",
    "  if [ -z \"${OPENCLAW_CHANNEL_CONTEXT:-}\" ]; then",
    "    echo \"OpenClaw origin session routing requires OPENCLAW_CHANNEL_CONTEXT.\" > \"$CRON_ADD_OUTPUT\"",
    "  elif ! openclaw sessions --json > \"$OPENCLAW_SESSIONS_OUTPUT\" 2> \"$CRON_ADD_OUTPUT\"; then",
    "    echo \"OpenClaw origin session routing could not list sessions.\" >> \"$CRON_ADD_OUTPUT\"",
    "  elif ! OPENCLAW_PARENT_SESSION_KEY=\"$(node - \"$OPENCLAW_CHANNEL_CONTEXT\" \"$OPENCLAW_SESSIONS_OUTPUT\" \"${OPENCLAW_AGENT:-main}\" <<'NODE' 2>> \"$CRON_ADD_OUTPUT\"",
    ...
    "}",
    "",
    "const contextRaw = process.argv[2];",
    "const sessionsPath = process.argv[3];",
    "const agentId = normalizeIdentity(process.argv[4]) || \"main\";",
    "let context;",
    "try {",
    "  context = JSON.parse(String(contextRaw || \"\"));",
    "} catch (error) {",
    "  fail(`OpenClaw origin session routing requires valid OPENCLAW_CHANNEL_CONTEXT JSON: ${String(error?.message || error)}`);",
    "}",
    "const contextObject = asObject(context);",
    "if (!contextObject) fail(\"OpenClaw origin session routing requires OPENCLAW_CHANNEL_CONTEXT to be a JSON object.\");",
    "const sender = asObject(contextObject.sender);",
    "const chat = asObject(contextObject.chat);",
    "const chatIdentity = normalizeIdentity(chat?.id);",
    "const senderIdentity = normalizeIdentity(sender?.id);",
    "const chosenIdentity = chatIdentity || senderIdentity;",
    "if (!chosenIdentity) {",
    "  fail(\"OpenClaw origin session routing requires chat.id or sender.id in OPENCLAW_CHANNEL_CONTEXT.\");",
    "}",
    "",
    "let sessionsPayload;",
    "try {",
    "  sessionsPayload = JSON.parse(readFileSync(sessionsPath, \"utf8\"));",
    "} catch (error) {",
    "  fail(`OpenClaw origin session routing could not parse openclaw sessions --json output: ${String(error?.message || error)}`);",
    "}",
    ...
    "  fail(\"OpenClaw origin session routing requires openclaw sessions --json to return a sessions array.\");",
    "}",
    "",
    "const collapsedMainRows = [...new Set(sessions.flatMap((session) => {",
    "  const sessionObject = asObject(session);",
    "  if (!sessionObject) return [];",
    "  const key = normalizeIdentity(sessionObject.key);",
    "  const sessionAgentId = normalizeIdentity(sessionObject.agentId);",
    "  const sessionKind = normalizeIdentity(sessionObject.kind);",
    "  if (sessionAgentId !== agentId || sessionKind !== \"direct\") return [];",
    "  return key === `agent:${agentId}:main` ? [key] : [];",
    "} ))];",
    "const candidateRoutes = sessions.flatMap((session) => {",
    "  const sessionObject = asObject(session);",
    "  if (!sessionObject) return [];",
    "  const key = normalizeIdentity(sessionObject.key);",
    "  const sessionAgentId = normalizeIdentity(sessionObject.agentId);",
    "  const sessionKind = normalizeIdentity(sessionObject.kind);",
    "  if (!key || !sessionAgentId || !sessionKind) return [];",
    "  if (sessionAgentId !== agentId) return [];",
    "  if (!key.startsWith(`agent:${agentId}:`)) return [];",
    "  if (sessionKind !== \"direct\" && sessionKind !== \"group\" && sessionKind !== \"channel\" && sessionKind !== \"room\") return [];",
    "  if (key.includes(\":run:\") || key.includes(\":cron:\")) return [];",
    "  const segments = key.split(\":\");",
    "  if (segments.length < 5) return [];",
    "  if (segments[0] !== \"agent\" || segments[1] !== agentId) return [];",
    "  if (!segments[2] || segments[3] !== sessionKind) return [];",
    "  const routeValue = segments.slice(4).join(\":\");",
    "  if (!routeValue) return [];",
    "  return [{ key, routeValue }];",
    "});",
    "const exactMatches = [...new Set(candidateRoutes.flatMap((route) => {",
    "  return route.routeValue === chosenIdentity ? [route.key] : [];",
    "} ))];",
    "const nestedConflicts = [...new Set(candidateRoutes.flatMap((route) => {",
    "  if (!chatIdentity) return [];",
    "  return route.routeValue.startsWith(`${chosenIdentity}:`) ? [route.key] : [];",
    "} ))];",
    "if (nestedConflicts.length > 0) {",
    "  fail(`OpenClaw origin session routing found nested channel routes for ${chosenIdentity}; refusing to guess between parent and thread/topic sessions.`);",
    "}",
    "",
    "if (exactMatches.length === 0 && collapsedMainRows.length > 0) {",
    "  fail(\"OpenClaw origin session routing found a collapsed direct main session key, but exact origin cannot be proven from agent:<agent>:main.\");",
    "}",
    "if (exactMatches.length !== 1) {",
    "  fail(`OpenClaw origin session routing expected exactly one matching session, found ${exactMatches.length}.`);",
    "}",
    "process.stdout.write(exactMatches[0]);",
    "NODE",
    "  )\"; then",
    "    :",
