...
function buildOpenClawInitialRunBootstrap({
  ...
  return [
    ...
    "  if (sessionKind !== \"direct\" && sessionKind !== \"group\") return [];",
    ...
    "  return lastSegment === chosenIdentity ? [route.key] : [];",
    "} ))];",
    "const nestedConflicts = [...new Set(candidateRoutes.flatMap((route) => {",
    "  if (!chatIdentity || route.segments.length <= 5) return [];",
    "  return route.segments.slice(4, -1).includes(chosenIdentity) ? [route.key] : [];",
    "} ))];",
