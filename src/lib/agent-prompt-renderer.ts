...
function buildOpenClawInitialRunBootstrap({
  ...
  return [
    ...
    "const identities = [...new Set([normalizeIdentity(chat?.id), normalizeIdentity(sender?.id)].filter(Boolean))];",
    ...
    "}",
    "",
    "const matches = [...new Set(sessions.flatMap((session) => {",
    ...
    "  if (!key || !sessionAgentId) return [];",
    "  if (sessionAgentId !== agentId) return [];",
    "  if (!key.startsWith(`agent:${agentId}:`)) return [];",
    "  if (key.includes(\":run:\")) return [];",
    "  const lastSegment = key.split(\":\").pop() || \"\";",
    "  if (!identities.includes(lastSegment)) return [];",
    "  return [key];",
    "} ))];",
    "",
    "if (matches.length !== 1) {",
    "  fail(`OpenClaw origin session routing expected exactly one matching session, found ${matches.length}.`);",
    "}",
