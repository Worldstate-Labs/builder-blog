...
- Read `task.contentStatus`.
  ...
  - For `requires_agent`, follow `task.fetchInstructions.prompt` as the
    ...
    fetch, local CLI tools yt-dlp/curl/ffmpeg, transcription APIs,
    headless browser, anything you have — until real primary content
    meeting task.minimumContentQuality is obtained). Do not override
    the prompt with your own heuristics.
